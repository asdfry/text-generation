...
for model_name in model_names:
    ...
    for decoding in ["greedy", "top_k", "top_p"]:
        for max_token in [1024, 2048, 4096]:
            ...
            for _ in tqdm(range(10)):
                rs = (
                    grequests.post(
                        url,
                        json={"sentences": ["What is deep learning?"], "max_token": max_token, decoding: True},
                        headers=headers,
                    )
                    for url in urls
                )
            ...
            logger.info(
                f"[Infer] decoding: {decoding}, token size: {max_token}, "
